...
// Helper function to parse formulas data
function parseFormulasData(content: string): Formula[] | null {
  try {
    console.log('🔍 Parsing formulas content...');

    // Extract just the formulas array content
    const arrayStart = content.indexOf('[');
    const arrayEnd = content.lastIndexOf(']');

    if (arrayStart === -1 || arrayEnd === -1) {
      console.error('Could not find formulas array boundaries');
      return null;
    }
    ...
    // Split by top-level objects (formulas)
    const pos = 1; // Skip opening bracket
    let currentObj = '';
    let braceCount = 0;
    let inString = false;
    let stringChar: string | null = null;

    for (let i = pos; i < arrayContent.length - 1; i++) {
      ...
      if (!inString) {
        ...
        if (char === '}') {
          braceCount--;
          if (braceCount === 0) {
            // We found a complete formula object
            try {
              const formula = parseFormulaObject(currentObj);
              if (formula) {
                formulas.push(formula);
              }
            } catch (e) {
              console.log(`  ⚠️  Failed to parse formula: ${e}`);
            }
            currentObj = '';
          }
        }
    ...
    console.log(`🔍 Successfully parsed ${formulas.length} formulas`);
    return formulas;

  } catch (error) {
    console.error('Failed to parse formulas data:', error);
    return null;
  }
}

// Helper function to parse individual formula objects
function parseFormulaObject(objStr: string): Formula | null {
  try {
    const obj: Partial<Formula> = {};
    ...
    // Extract formula
    const formulaMatch = objStr.match(/formula\s*:\s*['"]([^'"]+)['"]/);
    if (formulaMatch) obj.formula = formulaMatch[1];
    ...
    const variablesMatch = objStr.match(/variables\s*:\s*\[([\s\S]*?)\]/);
    if (variablesMatch) {
      const variablesStr = variablesMatch[1];
      const variables: Array<{symbol: string, description: string}> = [];

      // Parse variable objects
      const varRegex = /\{\s*symbol\s*:\s*['"]([^'"]+)['"]\s*,\s*description\s*:\s*['"]([^'"]+)['"]\s*\}/g;
      let varMatch;
    ...
    if (obj.id && obj.category && obj.name && obj.formula && obj.description) {
      return obj as Formula;
    }

    return null;
  } catch (error) {
    console.log(`Failed to parse formula object: ${error}`);
    return null;
  }
}

async function uploadAptitudeCourseData() {
  try {
    console.log('🚀 Starting Aptitude course data upload...');
    console.log('⚠️  This process may take several minutes. Please do not interrupt.\n');

    // First, ensure the Aptitude course exists
    const courseId = 'aptitude';
    let course = await prisma.course.findUnique({
      where: { courseId }
    });

    if (!course) {
      console.log('📚 Creating Aptitude course...');
      const aptitudeConfig = COURSES.aptitude;

      course = await prisma.course.create({
        data: {
          courseId: aptitudeConfig.id,
          title: aptitudeConfig.displayName,
          displayName: aptitudeConfig.displayName,
          description: 'Master quantitative aptitude skills for competitive exams and analytical thinking',
          headerTitle: aptitudeConfig.headerData.title,
          completionPercentage: aptitudeConfig.headerData.completionPercentage,
          showCodeEditor: aptitudeConfig.showCodeEditor,
          defaultModule: aptitudeConfig.defaultModule,
          autoSelectFirstTopic: aptitudeConfig.autoSelectFirstTopic,
          showCourseSwitcher: aptitudeConfig.showCourseSwitcher,
          monacoLanguage: aptitudeConfig.codeEditor?.monacoLanguage || null,
          codeDisplayName: aptitudeConfig.codeEditor?.displayName || null,
          defaultCode: aptitudeConfig.codeEditor?.defaultCode || null,
          executionLanguage: aptitudeConfig.codeEditor?.executionLanguage || null,
          executionVersion: aptitudeConfig.codeEditor?.executionVersion || null,
          aiAssistantName: aptitudeConfig.aiAssistant.name,
          aiAssistantDescription: aptitudeConfig.aiAssistant.description,
          aiAssistantPrompt: aptitudeConfig.aiAssistant.systemPrompt
        }
      });
      console.log('✅ Created Aptitude course');
    } else {
      console.log('✅ Aptitude course already exists');
    }

    // Get all module directories
    const modulesPath = path.join(process.cwd(), 'app', 'test', 'modules', 'aptitude');
    ...
    for (let batchStart = 0; batchStart < moduleDirs.length; batchStart += batchSize) {
      ...
      for (const moduleDir of batch) {
        try {
          ...
          // Process formulas
          const formulasPath = path.join(modulePath, 'formulas');
          if (fs.existsSync(formulasPath)) {
            const formulasIndexPath = path.join(formulasPath, 'index.ts');
            if (fs.existsSync(formulasIndexPath)) {
              const formulasContent = fs.readFileSync(formulasIndexPath, 'utf-8');

              const formulasData = parseFormulasData(formulasContent);
              if (formulasData && formulasData.length > 0) {
                console.log(`  📐 Processing ${formulasData.length} formulas...`);

                for (let i = 0; i < formulasData.length; i++) {
                  const formula = formulasData[i];

                  let courseFormula = await prisma.courseFormula.findFirst({
                    where: {
                      courseModuleId: courseModule.id,
                  ...
                  if (!courseFormula) {
                    courseFormula = await prisma.courseFormula.create({
                      data: {
                        courseModuleId: courseModule.id,
                        formulaId: formula.id,
                    ...
                    console.log(`  ✅ Formula ${formula.name} already exists`);
                  }
                }
              }
            }
    ...
    console.log('\n🎉 Aptitude course data upload completed successfully!');
    console.log('✅ All modules, topics, exercises, and questions have been processed.');
  } catch (error) {
    console.error('❌ Error uploading Aptitude course data:', error);
