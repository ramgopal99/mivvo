...
interface WritingSessionAttempt {
  id: string
  startedAt: Date
...
export function WritingResultsContent({ session }: WritingResultsContentProps) {
          ...
          <div className="text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-1">
                Complete writing practice sessions to see detailed analysis and progress tracking.
              </p>
            </div>
            <Button
  ...
  const getAttemptScore = (attempt: WritingSessionAttempt) => {
  ...
  const getAttemptAnalysis = (attempt: WritingSessionAttempt) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Avg Topic Writing</CardTitle>
            <PenTool className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{averageTopicWriting}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{averageConversation}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-semibold text-gray-900">Writing Session Attempts</h2>
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-600 text-center">
                You haven&apos;t completed any writing practice attempts yet.
              </p>
            </CardContent>
          </Card>
        ) : (
                      ...
                      if (hasAnyData) {
                        return (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => router.push(`/dashboard/foreign-lang/writing/${session.id}/attempt/${attempt.id}`)}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {hasScore && (
                            <div className="flex space-x-4">
                              ...
                              {analysis.topicWritingScore && (
                                <div className="text-center">
                                  <div className="text-lg font-semibold text-green-600">{Math.round(analysis.topicWritingScore)}</div>
                                  <div className="text-xs text-gray-600">Topic Writing</div>
                                </div>
                              )}
                              {analysis.aiConversationScore && (
                                <div className="text-center">
                                  <div className="text-lg font-semibold text-purple-600">{Math.round(analysis.aiConversationScore)}</div>
                                  <div className="text-xs text-gray-600">AI Conversation</div>
                                </div>
                              )}
                            </div>
                          )}
                      ...
                      <CardContent>
                        <div className="text-center py-4">
                          <div className="text-sm text-gray-500">
                            Writing session completed but no detailed analysis available.
                          </div>
                        </div>
                      </CardContent>
                    )
