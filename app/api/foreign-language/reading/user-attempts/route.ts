...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get reading attempts for the user, optionally filtered by language
    ...
    console.error('Error fetching user reading attempts:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fetch reading attempts' },
      { status: 500 }
    );
  }
}