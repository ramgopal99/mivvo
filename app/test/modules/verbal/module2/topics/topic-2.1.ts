import { SubLesson } from '../../../../data/lessonsData';

export const topic_2_1: SubLesson = {
  id: "2.1",
  title: 'Parts of Speech',
  status: 'completed',
  content: "`# ðŸ”¤ Parts of Speech: Building Blocks of English

Parts of speech are the fundamental categories into which words are classified based on their grammatical function. Understanding these eight parts of speech is essential for proper sentence construction and clear communication.

---

## ðŸ“‹ Overview of Parts of Speech

Every word in English belongs to one of eight parts of speech:

| Part of Speech | Function | Example |
|---|---|---|
| **Noun** | Names people, places, things, ideas | book, London, happiness |
| **Pronoun** | Replaces nouns | he, she, it, they |
| **Verb** | Shows action or state | run, is, have |
| **Adjective** | Describes nouns | big, red, beautiful |
| **Adverb** | Describes verbs, adjectives, other adverbs | quickly, very, here |
| **Preposition** | Shows relationship | in, on, at, with |
| **Conjunction** | Joins words/phrases | and, but, or, because |
| **Interjection** | Expresses emotion | wow, oh, hey |

---

## ðŸ·ï¸ 1. NOUNS

**Definition**: Words that name people, places, things, ideas, or qualities.

### Types of Nouns:

#### **Common Nouns** - General names
- **Examples**: book, city, dog, teacher, computer
- **Usage**: Not capitalized unless at sentence start

#### **Proper Nouns** - Specific names
- **Examples**: London, Microsoft, Shakespeare, Monday
- **Usage**: Always capitalized

#### **Abstract Nouns** - Ideas, qualities, states
- **Examples**: love, freedom, happiness, courage
- **Usage**: Cannot be sensed by five senses

#### **Collective Nouns** - Groups as single units
- **Examples**: team, family, herd, committee
- **Usage**: Can be singular or plural depending on context

**Examples in Sentences:**
- Common: The **student** reads a **book**.
- Proper: **Sarah** lives in **New York**.
- Abstract: **Peace** brings **happiness**.
- Collective: The **team** wins the **match**.

---

## ðŸ‘¥ 2. PRONOUNS

**Definition**: Words that replace nouns to avoid repetition.

### Types of Pronouns:

#### **Personal Pronouns**
| Subject | Object | Possessive |
|---|---|---|
| I | me | my/mine |
| you | you | your/yours |
| he | him | his |
| she | her | her/hers |
| it | it | its |
| we | us | our/ours |
| they | them | their/theirs |

#### **Reflexive Pronouns** - Refer back to subject
- **Examples**: myself, yourself, himself, herself, itself, ourselves, themselves
- **Usage**: Used when subject and object are the same

#### **Demonstrative Pronouns** - Point to specific things
- **Examples**: this, that, these, those
- **Usage**: Replace nouns and indicate distance/quantity

#### **Indefinite Pronouns** - Refer to nonspecific persons/things
- **Examples**: anybody, everybody, nobody, somebody, anyone, everyone, no one, someone
- **Usage**: Refer to people or things in general

**Examples:**
- Personal: **She** gave **her** **book** to **me**.
- Reflexive: He hurt **himself** while playing.
- Demonstrative: **This** is my pen, **that** is yours.
- Indefinite: **Everybody** needs **somebody** sometimes.

---

## âš¡ 3. VERBS

**Definition**: Words that express action, occurrence, or state of being.

### Types of Verbs:

#### **Action Verbs** - Show physical or mental action
- **Examples**: run, eat, think, sleep, write
- **Usage**: Can be transitive (need object) or intransitive

#### **Linking Verbs** - Connect subject to description
- **Examples**: is, am, are, was, were, be, being, been
- **Usage**: Followed by noun/pronoun/adjective describing subject

#### **Helping Verbs** - Help main verbs express tense/mood
- **Examples**: have, has, had, do, does, did, will, would, can, could
- **Usage**: Used with main verbs to form verb phrases

### Verb Tenses (Basic Forms):
- **Present**: walk, walks
- **Past**: walked
- **Future**: will walk
- **Present Continuous**: is walking
- **Past Continuous**: was walking

**Examples:**
- Action: The dog **barks** loudly.
- Linking: She **is** a teacher.
- Helping: He **has** finished his homework.

---

## ðŸŽ¨ 4. ADJECTIVES

**Definition**: Words that describe or modify nouns or pronouns.

### Types of Adjectives:

#### **Descriptive Adjectives** - Describe qualities
- **Examples**: beautiful, tall, red, happy, intelligent
- **Usage**: Tell what kind, how many, or which one

#### **Quantitative Adjectives** - Show quantity
- **Examples**: some, many, few, several, all
- **Usage**: Indicate amount or number

#### **Possessive Adjectives** - Show ownership
- **Examples**: my, your, his, her, its, our, their
- **Usage**: Indicate who owns something

#### **Demonstrative Adjectives** - Point out specific nouns
- **Examples**: this, that, these, those
- **Usage**: Specify which person/thing is being referred to

### Degrees of Adjectives:
- **Positive**: tall
- **Comparative**: taller
- **Superlative**: tallest

**Examples:**
- Descriptive: The **big**, **red** ball bounced high.
- Quantitative: **Many** people attended the **few** meetings.
- Possessive: **My** car is faster than **your** bike.
- Degrees: John is **tall**. Mike is **taller**. Tom is the **tallest**.

---

## ðŸƒ 5. ADVERBS

**Definition**: Words that modify verbs, adjectives, other adverbs, or entire sentences.

### Types of Adverbs:

#### **Adverbs of Manner** - How something is done
- **Examples**: quickly, slowly, carefully, loudly
- **Usage**: Answer "How?"

#### **Adverbs of Place** - Where something happens
- **Examples**: here, there, everywhere, nowhere
- **Usage**: Answer "Where?"

#### **Adverbs of Time** - When something happens
- **Examples**: now, then, today, yesterday, soon
- **Usage**: Answer "When?"

#### **Adverbs of Frequency** - How often something happens
- **Examples**: always, often, sometimes, never
- **Usage**: Answer "How often?"

#### **Adverbs of Degree** - To what extent
- **Examples**: very, quite, too, enough, almost
- **Usage**: Answer "To what extent?"

### Degrees of Adverbs:
- **Positive**: fast
- **Comparative**: faster
- **Superlative**: fastest

**Examples:**
- Manner: She sings **beautifully**.
- Place: Put it **there**.
- Time: He arrived **yesterday**.
- Frequency: I **always** brush my teeth.
- Degree: It's **too** hot **here**.

---

## ðŸ“ 6. PREPOSITIONS

**Definition**: Words that show relationships between nouns/pronouns and other words in a sentence.

### Common Prepositions:
- **Time**: at, on, in, before, after, during, since, until
- **Place**: in, on, at, under, above, behind, beside, between
- **Direction**: to, from, towards, through, across, along
- **Other**: with, by, for, about, of, as, like

### Prepositional Phrases:
- **Examples**: in the morning, on the table, with a friend, by car
- **Structure**: Preposition + Noun/Pronoun (+ Modifiers)

**Examples:**
- Time: The meeting is **at** 3 PM **on** Monday **in** January.
- Place: The book is **under** the table **beside** the chair.
- Direction: He walked **through** the park **towards** the river.
- Other: This gift is **for** you **from** your friend.

---

## ðŸ”— 7. CONJUNCTIONS

**Definition**: Words that join words, phrases, clauses, or sentences.

### Types of Conjunctions:

#### **Coordinating Conjunctions** - Join equal elements (FANBOYS)
- **Examples**: for, and, nor, but, or, yet, so
- **Usage**: Join independent clauses or words/phrases of equal importance

#### **Subordinating Conjunctions** - Join dependent and independent clauses
- **Examples**: although, because, since, unless, while, when, where, if
- **Usage**: Make one clause dependent on another

#### **Correlative Conjunctions** - Work in pairs
- **Examples**: both...and, either...or, neither...nor, not only...but also
- **Usage**: Join balanced elements

**Examples:**
- Coordinating: I like tea **and** coffee.
- Subordinating: I stayed home **because** it was raining.
- Correlative: **Both** pizza **and** pasta taste good.

---

## ðŸ˜² 8. INTERJECTIONS

**Definition**: Words or phrases that express strong emotion or surprise.

### Types of Interjections:

#### **Strong Emotions**
- **Examples**: Wow!, Oh!, Ah!, Ouch!, Oops!
- **Usage**: Express surprise, pain, pleasure, etc.

#### **Mild Expressions**
- **Examples**: Well, Hey, Hi, Hello, Goodbye
- **Usage**: Greetings, attention-getters

### Punctuation:
- **Examples**: Wow! That's amazing. Well, I think you're right.
- **Usage**: Usually followed by exclamation mark or comma

**Examples:**
- Strong: **Ouch**! That hurt. **Wow**! You're early.
- Mild: **Well**, what do you think? **Hey**, look at this!

---

## ðŸ§© IDENTIFICATION PRACTICE

**Identify the parts of speech in these sentences:**

1. **The** **quick** **brown** **fox** **jumps** **over** **the** **lazy** **dog**.
2. **She** **quickly** **ran** **to** **the** **store** **and** **bought** **milk**.
3. **Wow**! **That** **is** **really** **amazing**!

**Answers:**
1. The (article), quick (adjective), brown (adjective), fox (noun), jumps (verb), over (preposition), the (article), lazy (adjective), dog (noun).
2. She (pronoun), quickly (adverb), ran (verb), to (preposition), the (article), store (noun), and (conjunction), bought (verb), milk (noun).
3. Wow (interjection), That (demonstrative pronoun), is (linking verb), really (adverb), amazing (adjective)!

---

## ðŸ“ KEY TAKEAWAYS

1. **Nouns** name people, places, things, and ideas
2. **Pronouns** replace nouns to avoid repetition
3. **Verbs** show action or state of being
4. **Adjectives** describe nouns and pronouns
5. **Adverbs** modify verbs, adjectives, and other adverbs
6. **Prepositions** show relationships between words
7. **Conjunctions** join words, phrases, and clauses
8. **Interjections** express strong emotions

**Master these building blocks and you'll understand English sentence structure!** ðŸ—ï¸`"
};

