...
1. Open `app/test/course-config.ts`
...
### Core Components:
- **`app/test/course-config.ts`** - 🎯 **Single variable to switch courses!**
- **`utils/moduleLoader.ts`** - Dynamically loads modules based on language
- **`utils/headerDataLoader.ts`** - Dynamically loads header data based on language
...
```tsx
// 1. Change this line in app/test/course-config.ts:
...
<TestPage /> // Will use whatever is set in course-config.ts
```
...
See `app/test/examples.ts` for more usage patterns!
